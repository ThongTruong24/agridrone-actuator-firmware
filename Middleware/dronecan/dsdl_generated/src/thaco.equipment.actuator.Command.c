...
uint32_t _thaco_equipment_actuator_Command_encode(struct thaco_equipment_actuator_Command* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, THACO_EQUIPMENT_ACTUATOR_COMMAND_MAX_SIZE);
    __thaco_equipment_actuator_Command_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > THACO_EQUIPMENT_ACTUATOR_COMMAND_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (__thaco_equipment_actuator_Command_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct thaco_equipment_actuator_Command sample_thaco_equipment_actuator_Command_msg(void) {
    ...
    struct thaco_equipment_actuator_Command msg;
    ...
    msg.command_id = (uint16_t)random_bitlen_unsigned_val(16);
